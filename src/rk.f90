...
module mod_rk
  ...
  public rk,rk_scal
  contains
  subroutine rk(rkpar,n,dxci,dxfi,dyci,dyfi,dzci,dzfi,dlc_li_x,dlf_li_x,dlc_li_y,dlf_li_y,dlc_li_z,dlf_li_z,visc,dt,p, &
    ...
    real(rp), intent(in   ), dimension(0:)       :: dlc_li_x,dlf_li_x,dlc_li_y,dlf_li_y,dlc_li_z,dlf_li_z
    ...
    if(is_fast_mom_kernels) then
      ...
      if(.not.is_impdiff) then
        ...
        call momx_a(n(1),n(2),n(3),dxci,dyfi,dzfi,u,v,w,dudtrk)
        call momy_a(n(1),n(2),n(3),dxfi,dyci,dzfi,u,v,w,dvdtrk)
        call momz_a(n(1),n(2),n(3),dxfi,dyfi,dzci,u,v,w,dwdtrk)
      end if
    end if
    !
...
!#endif
    ...
    !
    call cmpt_bulk_forcing(n,is_forced,velf,dlc_li_x,dlf_li_x,dlc_li_y,dlf_li_y,dlc_li_z,dlf_li_z,u,v,w,f)
    !
    if(is_impdiff) then
      !
  ...
  !
  subroutine rk_scal(rkpar,n,l,dxci,dxfi,dyci,dyfi,dzci,dzfi,dlf_li_x,dlf_li_y,dlf_li_z,alpha,dt,is_bound,u,v,w, &
    ...
    real(rp), intent(in   ), dimension(0:) :: dxci,dxfi,dyci,dyfi,dzci,dzfi
    real(rp), intent(in   ), dimension(0:) :: dlf_li_x,dlf_li_y,dlf_li_z
    ...
    !
    if(is_forced) then
      call bulk_mean(n,dlf_li_x,dlf_li_y,dlf_li_z,s(:,:,:),mean)
      f = scalf - mean
    end if
    if(is_impdiff) then
      !
  ...
  !
  subroutine cmpt_bulk_forcing(n,is_forced,velf,dlc_li_x,dlf_li_x,dlc_li_y,dlf_li_y,dlc_li_z,dlf_li_z,u,v,w,f)
    implicit none
    integer , intent(in   ), dimension(3) :: n
    logical , intent(in   ), dimension(3) :: is_forced
    ...
    f(:) = 0.
    if(is_forced(1)) then
      call bulk_mean(n,dlc_li_x,dlf_li_y,dlf_li_z,u,mean)
      f(1) = velf(1) - mean
    end if
    if(is_forced(2)) then
      call bulk_mean(n,dlf_li_x,dlc_li_y,dlf_li_z,v,mean)
      f(2) = velf(2) - mean
    end if
    if(is_forced(3)) then
      call bulk_mean(n,dlf_li_x,dlf_li_y,dlc_li_z,w,mean)
      f(3) = velf(3) - mean
    end if
  end subroutine cmpt_bulk_forcing
  !
  subroutine cmpt_bulk_forcing_alternative(rkpar,n,l,dxci,dxfi,dyci,dyfi,dzci,dzfi, &
                                           visc,dt,is_bound,is_forced,u,v,w,tauxo,tauyo,tauzo,f,is_first)
    !
